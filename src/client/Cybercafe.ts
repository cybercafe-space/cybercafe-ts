...
export class Cybercafe {
  constructor(
    ...
    private baseUrl: string = "https://api.cybercafe.space"
  ) { }
  ...
  space(spaceId: string): Space {
    return new Space(this.baseUrl, this.apiKey, spaceId);
  }
 ...
 */
  async listSpaces(): Promise<
    { location: string; type: number; name: string | null }[]
  > {
    return request<
      { location: string; type: number; name: string | null }[]
    >(`${this.baseUrl}/v1/spaces`, {
      method: "GET",
      headers: this.apiKey ? { "x-api-key": this.apiKey } : {},
    });
  }
